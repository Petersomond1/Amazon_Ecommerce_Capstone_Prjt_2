...
//From the admindashboard
router.post('/api/row_ids', (req, res) => {
    const ids = req.body;
    // Store the ids in the server's memory or in a database
    // For this example, we'll just log them to the console
    console.log(ids);
    res.json({ message: 'Ids received' });
  });
...
function createProduct(req) {
    return {
        ...
        quantity: req.body.quantity,  //in cart
...
router.post('/api/add_to_cart', async (req, res, next) => {
    ...
    if (isProductAlreadyInCart) {
        const query = 'UPDATE cart SET quantity = quantity + ? WHERE product_id = ?';
        await db.query(query, [product.quantity, product.id]);
    } else {
        const query = 'INSERT INTO cart (product_id, quantity, price) VALUES (?,?,?)';
        await db.query(query, [product.id, product.quantity, product.sale_price || product.price]);
    }
